...
import {Input, Field, Box, Icon, Text, Link, Heading, ToastMessage} from "rimble-ui"
import DoDont from "../../components/documentation/DoDont";

# ToastMessage

Use toast messages to provide updates about a process. The `ToastMessage` component extends [Box](/box).

- *Want to find out about the provider?* Read [ToastMessage.provider documentation](/toast-message-provider)
...
`ToastMessage` uses all the style props from [Box](/box).
...
<ToastMessage variant="success" message={"Payment sent"} secondaryMessage={"You have 4.20 ETH remaining"} />

<ToastMessage variant="processing" message={"Transaction pending..."} secondaryMessage={"This might take a few minutes"} />

<ToastMessage variant="failure" message={"Transaction failed"} secondaryMessage={"You didn't have enough Ether"} />
...
*Note: it may be preferable to use alternative components like [`Modal`](/Develop/Modal) to bring a transaction failure to a user's attention, as they can't be missed and provide more space for explanation.*
...
- [`Modal`](/Develop/Modal) – you may want to use this component for transaction failures as it could be considered a critical information
