---
title: Tooltip
componentName: Tooltip
description: Use a Tooltip to give your user extra information about something on the interface. This component is a simple styled tooltip that uses react-popper.
image: "/images/Tooltip.png"
---

Use a `Tooltip` to give your user extra information about something on the interface. This component is a simple styled tooltip that uses [react-popper](https://github.com/d8660091/react-popper).

**Import component**

```
import { Tooltip } from 'rimble-ui';
```

## Examples

Different ways to use `Tooltip` in your product.

**Placement**

Adjust where your tooltip appears on hover

```js is=react-live
<Tooltip message="Top tooltip" placement="top">
  <Button.Outline>Show top</Button.Outline>
</Tooltip>
```

```js is=react-live
<Tooltip message="Left tooltip" placement="left">
  <Button.Outline>Show left</Button.Outline>
</Tooltip>
```

```js is=react-live
<Tooltip message="Bottom tooltip" placement="bottom">
  <Button.Outline>Show bottom</Button.Outline>
</Tooltip>
```

```js is=react-live
<Tooltip message="Right tooltip" placement="right">
  <Button.Outline>Show right</Button.Outline>
</Tooltip>
```

**Colour**

Choose the colour of your `Tooltip`.

```js is=react-live
<Tooltip message="Dark tooltip" placement="right" variant="dark">
  <Button mb={3}>Show dark</Button>
</Tooltip>
```

```js is=react-live
<Tooltip message="Light tooltip" placement="right" variant="light">
  <Button.Outline>Show light</Button.Outline>
</Tooltip>
```

```js is=react-live
<Tooltip message="Wrap me around your tooltip trigger">
  <Icon name="Info" />
</Tooltip>
```

## Props

<PropsTable propMetaData={props.propMetaData} />

** Styled System Props **

`Tooltip` uses `COMMON`, `TYPOGRAPHY` groups of [Styled System props](/components/rimble-ui/system-props).

## Design guidelines

Some best practices for using `Tooltip` in your product.

**Tooltips are for supporting information**

<DoDont>
  {{
    positive: {
      caption: "Use tooltips for extra information that some might find useful but don't necessarily need to be on the interface for everyone.",
      code: (
        <Box>
          <Tooltip
            placement="right"
            variant="dark"
            message="You're on the Rinkeby network"
          >
            <Button.Text
              icon="Check"
              mt={3}
            >
                Right network
            </Button.Text>
          </Tooltip>
        </Box>
      )
    },
    negative: {
      caption: "Tooltip information is hidden by default so shouldn't contain anything critical to someone using your product.",
      code: (
        <Box>
          <Tooltip
            placement="right"
            variant="dark"
            message="To use this dapp you must be on the Rinkeby network"
          >
            <Button.Text
              icon="Warning"
              mt={3}>
                Wrong network
            </Button.Text>
          </Tooltip>
        </Box>
      )
    }
  }}
</DoDont>

**Tooltips aren't labels**

<DoDont>
  {{
    positive: {
      caption: "Labels next to UI elements are always better than tooltips as it is not always clear that tooltips are there. And it's not always possible to hover.",
      code: (
        <Box>
          <Tooltip
            placement="bottom"
            variant="light"
            message="This will start the transaction"
          >
            <Button.Outline
              icon="Send"
              mt={3}
            >
              Send Ether
            </Button.Outline>
          </Tooltip>
        </Box>
      )
    },
    negative: {
      caption: "Don't use tooltips to explain UI elements that could be labelled. They're for elaboration not explanation.",
      code: (
        <Box>
          <Tooltip placement="bottom" variant="light" message="Send Ether">
            <Button.Outline
              icon="Send"
              mt={3}
            />
          </Tooltip>
        </Box>
      )
    }
  }}
</DoDont>

**Keep tooltips short**

Don't include too much text in a tooltip. They're for short clarification or elaboration. If you need more than a couple of sentences, consider using a [modal](/components/rimble-ui/Modal).

**Some suggested uses**

This will include a small <Tooltip message="This pays for your transaction to be added to the blockchain"><strong>transaction fee </strong></Tooltip> which we'll include in your transaction summary