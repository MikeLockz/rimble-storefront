...
Button's icon inherits all the props from [`Icon`](/components/rimble-ui/Icon), but also includes these:
...
`Button` uses all the style props from [`Box`](/components/rimble-ui/Box), but also includes these:
