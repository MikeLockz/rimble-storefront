...
Don't use toast messages for information that will directly impact the success of completing a task, for example a network failure. They dismiss automatically so things can be missed easily. Note: users are able to dismiss them immediately manually. For critical information, use [`Modal`](/components/rimble-ui/Modal) to bring that info to the user's attention.
